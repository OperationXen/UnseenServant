...
class BonusCredit(models.Model):
    ...
    credits = models.IntegerField(
        default=1, help_text="Number of additional credits", verbose_name="Number of bonus games"
    )
    expires = models.DateTimeField(
        null=True, blank=True, help_text="Date that these credits expire", verbose_name="Expiry date (optional)"
    )
    ...
    def __str__(self):
        return f"[{self.expires.date() if self.expires else ''}] {self.discord_name} ({self.credits})"
...
class Player(models.Model):
    ...
    game = models.ForeignKey(
        Game, related_name="players", on_delete=models.CASCADE, help_text="Game user is playing in"
    )
...
class Ban(models.Model):
    ...
    datetime_end = models.DateTimeField(
        blank=True, null=True, default=a_month_from_now, help_text="Ban expiry date/time"
    )
    issuer_id = models.CharField(null=True, blank=True, max_length=32, help_text="Discord ID of issuing moderator")
    issuer_name = models.CharField(max_length=32, help_text="Name of the issuing moderator")
    ...
    variant = models.TextField(
        max_length=2,
        choices=BanTypes.choices,
        default=BanTypes.HARD,
        help_text="'Hard Ban' removes existing signups, 'Soft' does not",
    )
