...
def _allocate_limited_admin_permissions(user):
    ...
    permissions = permissions + list(
        Permission.objects.filter(codename__in=["add_player", "view_player", "change_player", "delete_player"])
    )
    permissions = permissions + list(
        Permission.objects.filter(codename__in=["add_game", "view_game", "change_game", "delete_game"])
    )
    permissions = permissions + list(
        Permission.objects.filter(codename__in=["add_dm", "view_dm", "change_dm", "delete_dm"])
    )
    permissions = permissions + list(
        Permission.objects.filter(
            codename__in=["add_character", "view_character", "change_character", "delete_character"]
        )
    )
...
def _create_new_limited_admin_user(username, password):
    ...
    new_user_details = {"username": username, "email": None, "password": password, "is_staff": True}
