...
class PlayerBanEmbed(Embed):
    ...
    ban_names = {
        "ST": "Soft ban - cannot sign up for new games",
        "HD": "Hard ban - removed from all games",
        "PM": "Permanent ban",
    }
...
class BanPlayerView(View):
    ...
    async def update_timescale(self, select, interaction):
        ...
        if self.timeframe:
            await interaction.response.edit_message(
                content=f"Banning player [{self.user}] for {self.timeframe} days\nReason: {self.reason}"
            )
        else:
            await interaction.response.edit_message(
                content=f"Banning player [{self.user}] on a permanent basis\nReason: {self.reason}"
            )

    @discord.ui.button(label="Soft ban", style=discord.ButtonStyle.primary, row=1)
    async def softban(self, button, interaction):
        self.clear_items()
        await async_issue_player_ban(self.user, "ST", self.reason, self.ctx.author, self.timeframe)
        if self.timeframe > 0:
            await interaction.response.edit_message(
                content=f"Banned player [{self.user}] from further signups for {self.timeframe} days", view=self
            )
        else:
            await interaction.response.edit_message(
                content=f"Banned player [{self.user}] from further signups", view=self
            )
    ...
    async def hardban(self, button, interaction):
        self.clear_items()
        await async_issue_player_ban(self.user, "HD", self.reason, self.ctx.author, self.timeframe)
        if self.timeframe > 0:
            await interaction.response.edit_message(
                content=f"Banned player [{self.user}] for {self.timeframe} days, and removed them from all outstanding games",
                view=self,
            )
        else:
            await interaction.response.edit_message(
                content=f"Banned player [{self.user}], and removed them from all outstanding games", view=self
            )
        await self.notify_user()
