...
@bot.slash_command(
    guild_ids=DISCORD_GUILDS, description="Force remove a player from a game"
)
@has_any_role(*DISCORD_DM_ROLES, *DISCORD_ADMIN_ROLES)
async def remove_player(
    ctx, user: Option(Member, "Player to remove from the game", required=True)
):
    """remove a player from a game - should only be run in a game channel"""
    await ctx.response.defer(ephemeral=True)
    log.info(
        f"{ctx.author.name} used command /remove_player {user.name} in channel {ctx.channel.name}"
    )
    ...
    if not game:
        ...
        return await ctx.followup.send(
            "This channel is not linked to a game", ephemeral=True
        )

    if not do_dm_permissions_check(ctx.author, game):
        return await ctx.followup.send(
            "You are not the DM for this game", ephemeral=True
        )
...
@bot.slash_command(
    guild_ids=DISCORD_GUILDS, description="Forcibly add a player to a game"
)
@has_any_role(*DISCORD_DM_ROLES, *DISCORD_ADMIN_ROLES)
async def add_player(
    ctx, user: Option(Member, "Player to add to the game", required=True)
):
    """add a player to a game - should only be run in a game channel"""
    await ctx.response.defer(ephemeral=True)
    log.info(
        f"{ctx.author.name} used command /add_player {user.name} in channel {ctx.channel.name}"
    )
    game = await async_get_game_for_channel(ctx.channel)
    if not game:
        log.error(f"Channel {ctx.channel.name} has no associated game, command failed")
...
@bot.slash_command(
    guild_ids=DISCORD_GUILDS, description="Forcibly add a player to a game waitlist"
)
@has_any_role(*DISCORD_DM_ROLES, *DISCORD_ADMIN_ROLES)
async def add_waitlist(
    ctx, user: Option(Member, "Player to add to the waitlist", required=True)
):
    """add a player to a games waitlist - should only be run in a game channel"""
    await ctx.response.defer(ephemeral=True)
    log.info(
        f"{ctx.author.name} used command /add_waitlist {user.name} in channel {ctx.channel.name}"
    )
    game = await async_get_game_for_channel(ctx.channel)
    if not game:
        log.error(f"Channel {ctx.channel.name} has no associated game, command failed")
...
@bot.slash_command(
    guild_ids=DISCORD_GUILDS, description="Tags all players in the party"
)
@has_any_role(*DISCORD_DM_ROLES, *DISCORD_ADMIN_ROLES)
async def tag_players(ctx):
    ...
    await ctx.response.defer(ephemeral=True)
    log.info(
        f"{ctx.author.name} used command /tag_players in channel {ctx.channel.name}"
    )
    game = await async_get_game_for_channel(ctx.channel)
    if not game:
        log.error(f"Channel {ctx.channel.name} has no associated game, command failed")
    ...
    return await ctx.followup.send(
        "Party have been individually tagged", ephemeral=True
    )
...
async def warn_waitlist(ctx):
    ...
    await ctx.response.defer(ephemeral=True)
    log.info(
        f"{ctx.author.name} used command /warn_waitlist in channel {ctx.channel.name}"
    )
    game = await async_get_game_for_channel(ctx.channel)
    if not game:
        log.error(f"Channel {ctx.channel.name} has no associated game, command failed")
    ...
    try:
        ...
        return await ctx.followup.send(
            f"Player {discord_user.display_name} notified", ephemeral=True
        )
    except Exception as e:
        log.error(f"Unable to find waitlisted player")
        return await ctx.followup.send(
            "Unable to message player at top of waitlist", ephemeral=True
        )
