...
class MusteringView(View):
    ...
    async def msc(self, interaction):
        ...
        if characters:
            ...
            view.message = await interaction.followup.send(f"Characters for {interaction.user} from the Moonsea Codex:", view=view, ephemeral=True)
        else:
            message = await interaction.followup.send(f"Cannot find any characters for you on Moonsea Codex, have you set your discord profile ID?")
